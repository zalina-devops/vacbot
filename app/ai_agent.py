...
def adapt_resume(vacancy_id):
    ...
    # Системный промпт для задания роли
...
Отвечай строго в формате Markdown с двумя разделами: "## О себе" и "## Ключевые навыки".
НЕ выдумывай навыки, которых нет в профиле.
ВСЕГДА завершай ответ полностью, не обрывай на полуслове."""

    prompt = f"""Адаптируй резюме под следующую вакансию.
    Формат вывода:
НЕ используй Markdown (##, **, -). Пиши простым текстом с абзацами.
...
7. НЕ используй Markdown (##, **, -). Пиши простым текстом с абзацами.
...
Выведи ПОЛНЫЙ ответ в формате:

## О себе
[3-4 полных предложения, адаптированные под вакансию]

## Ключевые навыки
- [навык 1: как применяла в проектах]
- [навык 2: как применяла в проектах]
- [навык 3: уровень владения]
- [навык 4: уровень владения]

Заверши ответ полностью, не обрывай на полуслове."""

    return call_openrouter(prompt, system_prompt=system_prompt, max_tokens=2000)
...
ВСЕГДА завершай письмо полностью."""
...
Заверши письмо полностью."""

    return call_openrouter(prompt, system_prompt=system_prompt, max_tokens=1500)
